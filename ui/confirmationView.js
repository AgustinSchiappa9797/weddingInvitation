...
export function renderConfirmation(els, data, options = {}) {
    ...
    const defaultCount = Math.min(companions, Math.max(1, Number(data?.companions) || 1));
    const closed = isConfirmationClosed(data);

    els.confirmationForm.dataset.token = options.token || "";

    populateCountOptions(els.confirmationCount, companions, defaultCount);
